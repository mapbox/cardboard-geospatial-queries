...
module.exports = function(bbox) {
  ...
  var tiles = bboxes.map(function(bbox) {
    ...
    if (memo[0][id] === undefined) memo[0][id] = 1;
    return memo;
  }, [{}]).reduce(function(tiles, memo) {
    return Object.keys(memo);
  }, []).map(function(id) { return id.split('!'); });
  ...
  return tiles.map(function(tile) { return tilebelt.tileToQuadkey(tile); })
    .concat(matchTiles.map(function(tile) { return tilebelt.tileToQuadkey(tile)+'!'; }));
};
